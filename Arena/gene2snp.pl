...
# form command to return all dbSNP information for list of dbSNP IDs
...
# retrieve the SNP information
my $sxresponse = $ua->request($sxreq);
my $sxop= $sxresponse->content;

my $sfile = $geneid . '_snp.txt';
my $ifile = $geneid . '_info.txt';
...
print SNPF "SNP_ID\tdbSNP\tGene_ID\tType\tCoding\tClinical\tPubmed\t",
...
# that pertains to amino acid substitution
	...
	foreach my $protid (keys %protst) {
		...
		elsif (exists $protst{$protid}{'stop-gained'}) {
			my $resch = $protst{$protid}{'reference'}{'res'} . '->' .
		...
		elsif (exists $protst{$protid}{'stop-lost'}) {
			my $resch = $protst{$protid}{'reference'}{'res'} . '->' .
		...
		elsif (exists $protst{$protid}{'frameshift-variant'}) {
			my $resch = $protst{$protid}{'reference'}{'res'} . '->' .
